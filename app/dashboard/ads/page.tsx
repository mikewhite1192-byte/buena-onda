...
function AdCreatorOverlay({ client, onClose }: {
  ...
  useEffect(() => {
    ...
    const prompts: Record<AdPlatform, string> = {
      ...
      tiktok: `I want to create a new TikTok Ads campaign for ${client.name}, a ${vertical} client. Rules: ask ONE question at a time — never combine questions. Gather: campaign objective (Traffic/Conversions/Lead Gen), daily budget, target audience (age, interests, location), video creative URL, ad copy. Present the full summary and get explicit confirmation before creating. Never create anything without their go-ahead.`,
      shopify: `I want to create product ads for ${client.name}'s Shopify store. Rules: ask ONE question at a time — never combine questions. Gather: which products to promote, campaign objective (Sales/Traffic), daily budget, target audience, landing page (product or collection page URL). Present the full summary and get explicit confirmation before creating. Never create anything without their go-ahead.`,
    };
